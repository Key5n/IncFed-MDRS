...
from experiments.utils.scaffold import update_c_global
import torch
...
if __name__ == "__main__":
    result_dir = os.path.join("result", "tranad", "scaffold")
    os.makedirs(result_dir, exist_ok=True)
    init_logger(os.path.join(result_dir, "scaffold-tranad.log"))
    ...
    with logging_redirect_tqdm():
        for global_round in trange(global_epochs):
            ...
            data_nums: list[int] = []
            client_update_list: list[Dict] = []

            for client in active_clients:
                logger.info(client.client_name)
                next_state_dict, data_num, client_update = client.train_scaffold(
                    global_state_dict, c_global
                )

                next_state_dict_list.append(next_state_dict)
                client_update_list.append(client_update)
                data_nums.append(data_num)

            c_global = update_c_global(c_global, client_update_list, len(clients))
